import { 网卡状态, 网卡类, 节点类, 链路层报文类, 频道类 } from "@/网络模型/网络";
import {离散事件仿真器} from "@/网络模型/离散事件仿真器";
import { 事件类, 注册方法调用事件 } from "@/事件";

export class 轮询网卡类 extends 网卡类<轮询频道类> {
  static readonly 实例化事件 = new 事件类<[轮询网卡类]>("轮询网卡类");
  protected static _类型名称 = "轮询";
  时隙剩余长度 = 0;
  ...
  protected 传输(数据包: 链路层报文类): void {
    ...
    if (传输时延 <= this.时隙剩余长度) {
      ...
      if (this.频道) {
        离散事件仿真器.添加事件(传输时延, this.频道.广播.bind(this.频道, this, 数据包));
      }
    } else {
  ...
  protected 传输中断数据包(): void {
    ...
    if (传输时延 <= this.时隙剩余长度) {
      ...
      if (this.频道) {
        离散事件仿真器.添加事件(传输时延, this.频道.广播.bind(this.频道, this, this.中断数据包!));
      }
      this.中断数据包 = null;
      this.中断数据包剩余长度 = null;
    } else {
...
export class 轮询频道类 extends 频道类<轮询网卡类> {
  static readonly 实例化事件 = new 事件类<[轮询频道类]>("轮询频道类");
  readonly 网卡类型 = 轮询网卡类;
  时隙长度: number;
